...
THIS FILE DOES NOT EXPECT A BALL BERRY PARAMETER
...
//  double wue; // water use efficiency, in mg CO2 fixed * g^-1 H20 transpired
  ...
  if (potGrossPsn < TINY) { // avoid divide by 0
    ...
    *dWater = 1; // dWater doesn't matter, since we don't have any photosynthesis
                     ...
                     * m^2 
                     * */;
		gs_canopy = M * potGrossPsn/12*1000*lai_int * (RH_pcent/CO2_stom) ;
		//mol  / m^2 leaf area 
 ...
 */ 
  if (potGrossPsn < TINY) { // avoid divide by 0
    ...
    *dWater = 1; // dWater doesn't matter, since we don't have any photosynthesis
    ...
    potTrans = (DELTA*(climate->par - G) + (RHO*CP*vpd)/(rCanConst/climate->wspd))/(DELTA+GAMMA*(1 + (params.rdConst/climate->wspd)/(rCanConst/climate->wspd)));
...
// #else
#if PENMAN_MONTEITH_TRANS // If the Penman Monteith option is being used...
