...
THIS FILE EXPECTS A BALL BERRY PARAMETER
m_ballberry
...
// some constants for Ball Berry and Penman Monteith submodel:
#define M 3.89 // the assumed value of the BallBerry Slope
#define DELTA 0.2 // number plucked out of the air... slope of VPD vs T for Penman Monteith Equation
...
#define NUM_PARAMS 55 // this only counts parameters read from file
...
/* Read climate file into linked lists, 
   ...
   location year day time intervalLength tair tsoil par precip vpd vpdSoil vPress wspd soilWetness netrad
...
void outputState(FILE *out, int loc, int year, int day, double time) {
  fprintf(out, "%8d %4d %3d %5.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.3f %8.2f %8.2f %8.2f %8.8f %8.2f %8.3f %8.3f  %8.3f \n", 
	  loc, year, day, time,
	  envi.plantWoodC, envi.plantLeafC, envi.soil, envi.litter, envi.litterWater, envi.soilWater, trackers.soilWetnessFrac, envi.snow,
	  trackers.npp, trackers.yearlyNpp, trackers.nee, trackers.totNee, trackers.evapotranspiration, fluxes.transpiration, trackers.gpp );
}
...
//not used   double wue; // water use efficiency, in mg CO2 fixed * g^-1 H20 transpired
  double rCanConst = 200; // wild guess;
 ...
 */ 
  if (potGrossPsn < TINY) { // avoid divide by 0
    ...
    *dWater = 1; // dWater doesn't matter, since we don't have any photosynthesis
    ...
    potTrans = (DELTA*(climate->netrad - climate->soilheat) + (RHO*CP*vpd)/(rCanConst/climate->wspd))/(DELTA+GAMMA*(1 + (params.rdConst/climate->wspd)/(rCanConst/climate->wspd)));
...
#if ALTERNATIVE_TRANS // IF WE ARE NOT USING THE DEFAULT TRANSPIRATION CALCULATION

	#if BALL_BERRY // if the Ball Berry option is being used...
  		moisture_bwb(&(fluxes.transpiration), &dWater, potGrossPsn, climate->vpd, climate->vPress, envi.plantLeafC, params.leafCSpWt, soilWater);
 	#else
 		// If the Penman Monteith option is being used...BallBerry must be set to off
 		moisture_pm(&(fluxes.transpiration), &dWater, potGrossPsn, climate->vpd, soilWater);
		//check the inputs and calculated variables...
	#endif
#else
