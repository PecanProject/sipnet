...
int main(int argc, char *argv[]) {
  ...
  FILE *out, *pChange;
  char line[8192];  // allow this to be long, since it might have to store lots
                    // of parameter names
  char *errc;
  ...
  int statsOnly = 0;  // do we only output means & standard dev's for a
                      // montecarlo run?
  int numChangeableParams, i;  // in pChange
  int *indices;  // indices of changeable params
  char *paramName;  // name of one of the parameters that varies for a
                    // montecarlo run
  ...
  char mcParamFile[FILE_MAXNAME],
      mcOutFileBase[FILE_MAXNAME];  // used for runtype=montecarlo
  int runNum;

  // variables used for outputting means/standard deviations over a set of
  // parameter sets:
  int j, k, type;
  double **model;
  double **means;
  ...
  /* and make sure we read everything we needed to:
     (note that a few variables had default values, so it's okay if they weren't
  ...
  if ((strcmpIgnoreCase(runtype, "montecarlo") == 0) && (statsOnly == 1)) {
    doMainOutput = 1;  // it's silly for this to be false: means & standard
                       // dev's wouldn't be output!
  ...
  if (strcmpIgnoreCase(runtype, "standard") == 0) {  // do a single run
  ...
  else if (strcmpIgnoreCase(runtype, "montecarlo") == 0) {  // multiple runs
                                                            // from file
    if (loc == -1) {
      printf("loc was set to -1: can only run multiple runs from file at one "
             "location: running at location 0\n");
      loc = 0;
    }
    ...
    strtok(line, " \t\n\r");  // read and ignore first token -- split on space,
                              // tab, newline & carriage return
    numChangeableParams = 1;  // assume at least one changeableParam
    while (strtok(NULL, " \t\n\r") != NULL) {  // now count # of remaining
                                               // tokens (i.e. # of parameter
                                               // names)
      numChangeableParams++;
    }
    ...
    for (i = 0; i < numChangeableParams; i++) {
      ...
      paramName = strtok(NULL, " \t\n\r"); /* get the next item (note: the last
            time this is called, we'll have paramName = NULL; that's okay,
    ...
    if (!statsOnly) {
      ...
      runNum = 1;
      out = NULL;
      while ((fgets(line, sizeof(line), pChange) != NULL) &&
             (strcmp(line, "\n") != 0) && (strcmp(line, "\r\n") != 0)) {
        ...
        // set value of changeable parameter #0
        setSpatialParam(spatialParams, indices[0], loc, value);
      ...
      // do each run, only output means and standard devs. of NEE to
      // mcOutFileBase.out: first pass: compute means; second pass: compute
      // standard deviations
      ...
      for (k = 0; k < 2; k++) {
        ...
        while ((fgets(line, sizeof(line), pChange) != NULL) &&
          ...
          if (numToSkip > 0) {
            strtok(line, " \t");  // read and ignore first value
            for (i = 0; i < numToSkip - 1; i++) {  // read and ignore other
                                                   // values
              strtok(NULL, " \t");
            }
            // now get first real value:
          ...
          // do this model run:
          runModelNoOut(model, MAX_DATA_TYPES, dataTypeIndices, spatialParams,
                        loc);
          ...
          if (k == 0) {  // first pass
            ...
            for (j = 0; j < steps[0]; j++) {
              for (type = 0; type < MAX_DATA_TYPES; type++) {
                standarddevs[j][type] +=
                    pow((model[j][type] - means[j][type]), 2);
              }
            }
          }
        ...
        if (k == 0) {  // first pass
          ...
          for (j = 0; j < steps[0]; j++) {
            for (type = 0; type < MAX_DATA_TYPES; type++) {
              standarddevs[j][type] =
                  sqrt(standarddevs[j][type] / (double)(runNum - 1));
            }
          }

          if (doMainOutput) { /* Note: it would be silly for doMainOutput to be
               false, because if it were, the means and standard deviations
               would not be output! */
  ...
  else if (strcmpIgnoreCase(runtype, "senstest") == 0) {  // do a sensitivity
                                                          // test
    if (doMainOutput) {
      strcpy(outFile, fileName);
      strcat(outFile, ".sens");
    ...
    if (loc == -1) {
      printf("loc was set to -1: can only run sens. test at one location: "
    ...
    sensTest(out, outputItems, changeIndex, lowVal, highVal, numRuns,
             spatialParams, loc);
    if (doMainOutput) {
      fclose(out);
    }
