...
// includes definition of MeanTracker structure that is central to the
// computation of a running mean
...
typedef struct MeanTrackerStruct {
  ...
  double totWeight;  // at any time, sum of all weights of stored values =
                     // totWeight
  ...
  // we store this so it is easy to re-compute the sum when we insert a new
  // value
} MeanTracker;
