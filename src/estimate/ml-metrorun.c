...
/* PRE: dataTypeSwitches is of size MAX_DATA_TYPES
   dataTypeSwitches[i] is 1 if we are including this data type in optimization,
   0 if not dataTypeIndices must be big enough to hold MAX_DATA_TYPES ints

   POST: dataTypeIndices[0..numDataTypes-1] contains the indices of the 1-valued
   elements in dataTypeSwitches
...
void printDataTypeIndices(int dataTypeIndices[], int numDataTypes,
                          FILE *filePtr) {
  int i;
...
void printDataTypeWeightIndices(int dataTypeIndices[], int numDataTypes,
                                double dataTypeWeights[], FILE *filePtr) {
  int i;
  char **dataTypeNames = getDataTypeNames();
  ...
  for (i = 0; i < numDataTypes; i++) {
    fprintf(filePtr, "\t%s=%2.2f\n", dataTypeNames[dataTypeIndices[i]],
            dataTypeWeights[dataTypeIndices[i]]);
  }
}
...
int main(int argc, char *argv[]) {
  ...
  // extension of optional file holding indices for model-data comparisons
  char compareIndicesExt[FILE_MAXNAME] = COMPARE_INDICES_EXT;
  // optional file holding indices for model-data comparisons
  char compareIndicesFile[2 * FILE_MAXNAME];
  // extension of optional file holding indices for optimizations
  char optIndicesExt[FILE_MAXNAME] = OPT_INDICES_EXT;
  // optional file holding indices for optimizations
  char optIndicesFile[2 * FILE_MAXNAME];
  // extension of optional file for aggregating
  char aggregationExt[FILE_MAXNAME] = AGGREGATION_EXT;
  char aggregationFile[2 * FILE_MAXNAME];  // optional file for aggregating
  // if, after getting optional arguments, paramFile is "", set
  // paramFile = fileName.param
  char paramFile[FILE_MAXNAME] = PARAM_FILE;
  long iter = ITER, numSpinUps = NUM_SPIN_UPS;
  int numChains = NUM_CHAINS, numRuns = NUM_RUNS, numAtOnce = NUM_AT_ONCE,
      loc = LOC;
  int randomStart = RANDOM_START;
  double unaggedWeight = UNAGGED_WEIGHT, scaleFactor = SCALE_FACTOR,
         paramWeight = PARAM_WEIGHT;
  double validFrac = VALID_FRAC;
  int dataTypeIndices[MAX_DATA_TYPES];  // MAX_DATA_TYPES defined in sipnet.c
  int dataTypeSwitches[MAX_DATA_TYPES];  // 0 or 1 for each data type; used to
                                         // build dataTypeIndices array
  double dataTypeWeights[MAX_DATA_TYPES];  // Likelihood Weights for each data
                                           // type we are optimizing on
  ...
  char paramOutFile[FILE_MAXNAME + 48],
      spatialParamOutFile[FILE_MAXNAME + 48];  // for outputting best parameters
  void *differenceFunc = difference; /* the difference function to use (depends
          on whether we're aggregating model & data) (default is plain old
          vanilla "difference") */
  char **dataTypeNames;
  char optTypeName[NAMELIST_INPUT_MAXNAME];  // names such as OPT_NEE, read in
                                             // from input file
  char weightTypeName[NAMELIST_INPUT_MAXNAME];  // names such as WEIGHT_NEE,
  ...
  for (i = 0; i < MAX_DATA_TYPES; i++) {
    if (strlen(dataTypeNames[i]) + 4 >= NAMELIST_INPUT_MAXNAME) {
      printf("ERROR: OPT_%s is too long of a name for namelist input\n",
             dataTypeNames[i]);
      printf("Either change the name of this data type, or increase "
             "NAMELIST_INPUT_MAXNAME in namelistInput.h\n");
      exit(1);
    }
    strcpy(optTypeName, "OPT_");
    strcat(optTypeName, dataTypeNames[i]);

    addNamelistInputItem(namelistInputs, optTypeName, INT_TYPE,
                         &(dataTypeSwitches[i]), 0);
    dataTypeSwitches[i] = 0;  // initialize to 0 in case it's not read from
                              // input file
  }
  ...
  for (i = 0; i < MAX_DATA_TYPES; i++) {
    ...
    addNamelistInputItem(namelistInputs, weightTypeName, DOUBLE_TYPE,
                         &(dataTypeWeights[i]), 0);
    dataTypeWeights[i] = 0;  // initialize to 0 in case it's not read from input
                             // file
  }
  ...
  /* and make sure we read everything we needed to:
     (note that many variables had default values, so it's okay if they weren't
  ...
  if (numDataTypes == 0) {
    printf("ERROR: Must specify at least one data type to include in "
           "optimization\n");
  ...
  printModelComponents(userOut);  // write to file which model componenets are
                                  // turned on
  ...
  printDataTypeWeightIndices(dataTypeIndices, numDataTypes, dataTypeWeights,
                             userOut);
  fprintf(userOut, "\n\n");

  readData(inFileName, dataTypeIndices, numDataTypes, MAX_DATA_TYPES, numLocs,
           steps, validFrac, optIndicesFile, compareIndicesFile, userOut);

  if (strcmp(aggregationFile, "") != 0) {  // there is a file for model-data
                                           // aggregation
    readFileForAgg(aggregationFile, numDataTypes, unaggedWeight);
    differenceFunc = aggedDifference;
    /* A bifurcation of data types; each data type is split into two data types:
     *  an aggregated and an unaggregated. But this bifurcation only applies to
     *  the collection of statistics, not to the actual running of the model!
  ...
  for (runNum = 1; runNum <= numRuns; runNum++) {
    ...
    metropolis(thisFile, spatialParams, loc, differenceFunc, runModelNoOut,
               ...
               paramWeight, scaleFactor, dataTypeIndices, numDataTypes,
               costFunction, dataTypeWeights, userOut);
