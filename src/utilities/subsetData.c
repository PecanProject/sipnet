/* subsetData: A stand-alone program
   ...
   - Right now, set up to do no aggregation ("none") or daily aggregation
     ("daily")

   Note that you could use this program to just create desired .spd and .agg
   files without doing any subsetting (START_YEAR = START_DAY = NUM_DAYS = -1)
...
#define MAX_LINE                                                               \
  1024  // maximum line length for input files, including
        // terminating '\0'
...
*/
char *readClimLine(FILE *f, char *line, int maxLength, int *loc, int *year,
                   int *day) {
...
/* Make sure climFin only contains data for a single location
   ...
   climFin should be open for reading and rewound; it is rewound before exiting
   this function
 */
void checkLocations(FILE *climFin) {
  ...
  while (readClimLine(climFin, line, MAX_LINE, &loc, &year, &day) !=
         NULL) {  // while not EOF or error
    if (loc != 0) {
      printf("ERROR: Read loc = %d, but input climate file must contain only "
             "location 0\n",
             loc);
      exit(1);
    }
...
/* Read lines from clim, data, valid and sigma files,
   ...
   startYear and startDay may be modified to hold the ACTUAL starting year and
   day (which may be different from the values passed in - for example if these
   values are -1, indicating that we should just start at the beginning)
...
*/
void gotoStart(FILE *climFin, FILE *dataFin, FILE *validFin, FILE *sigmaFin,
               int *startYear, int *startDay) {
  char line[MAX_LINE];
  ...
  while (!pastStart(year, day, *startYear, *startDay)) {
    status = fgets(line, MAX_LINE, dataFin);  // read & ignore line from data
                                              // file
    checkStatus(status, "ERROR reading data file: Unexpected EOF or error\n");
    status = fgets(line, MAX_LINE, validFin);  // read & ignore line from valid
                                               // file
    checkStatus(status, "ERROR reading valid file: Unexpected EOF or error\n");
    status = fgets(line, MAX_LINE, sigmaFin);  // read & ignore line from valid
                                               // file
    ...
    status = readClimLine(climFin, line, MAX_LINE, &loc, &year, &day);
    checkStatus(status, "ERROR reading climate file: reached EOF before "
                        "finding desired start date\n");
  }  // while !pastStart
...
/* Starting at current positions in climate, data, and valid input files,
   copy from input files to output files until we've read numDays days
   (numDays <= 0 indicates that we should read until end of file).
   Also create spdFout (steps per day; including leading startYear, startDay)
   and aggFout (# steps per aggregation time step)
    - Aggregation time step determined by aggType (must be one of the
      AGGREGATION_TYPES)
    - Right now, only implemented for NO_AGGREGATION (1 step per aggregation
      step) and DAILY_AGGREGATION
 ...
 */
void copySubset(FILE *climFin, FILE *dataFin, FILE *validFin, FILE *sigmaFin,
                FILE *climFout, FILE *dataFout, FILE *validFout,
                FILE *sigmaFout, FILE *spdFout, FILE *aggFout, int aggType,
                int startYear, int startDay, int numDays) {
  char line[MAX_LINE];
  int loc, year, day;
  ...
  status = readClimLine(climFin, line, MAX_LINE, &loc, &year, &day);
  checkStatus(status,
              "ERROR reading climate file: no data found after start date\n");
  ...
  while (!done) {
    ...
    if (status == NULL) {  // EOF or error
      ...
      if ((numDays <= 0) || (ndays == numDays)) {
        // If we have hit end of file or read enough days
        done = 1;
      } else {  // we haven't read enough days
        printf("ERROR reading climate file: reached EOF or error after reading "
               "only %d of %d days\n",
               ndays, numDays);
        printf("Try choosing an earlier start date, or a smaller number of "
               "days\n");
        exit(1);
      }
    }
      ...
      if ((year - currYear > 1) ||
          ((year - currYear == 1) && (currDay < 365 || day > 1)) ||
          ((year == currYear) && (day - currDay) > 1)) {
        // if we (1) advanced by more than 1 year, or (2) advanced by one year,
        // but we either (a) weren't on day 365 (or 366), or (b) now aren't on
        // day 1, or (3) advanced by more than one day (note: we don't
        // explicitly check for climate going backwards: we assume this doesn't
        // happen)
        printf("ERROR: subsetData only implemented for input files with at "
               "least one step per day\n");
        exit(1);
      }
      ...
      if (day != currDay) {
        ...
        if ((numDays > 0) && (ndays > numDays)) {  // we've read numDays days
          ...
          /* output end-of-line terminators to aggregation and spd files
             Note that, because the only possible aggregations are
             NO_AGGREGATION and DAILY_AGGREGATION, we will definitely have
             already output the last agg. step to aggregation file. If we add
             another aggregation type (e.g. annual), we may need to output the
             last value here before terminating the line
          */
          fprintf(aggFout, "-1\n");
          fprintf(spdFout, "-1\n");
        }
...
int main(int argc, char *argv[]) {
  ...
  FILE *climFin, *dataFin, *validFin, *sigmaFin, *climFout, *dataFout,
      *validFout, *sigmaFout, *spdFout, *aggFout;
  ...
  // do the work:
  checkLocations(climFin);  // make sure the climate file only contains one
                            // location
  gotoStart(climFin, dataFin, validFin, sigmaFin, &startYear, &startDay);
  copySubset(climFin, dataFin, validFin, sigmaFin, climFout, dataFout,
             validFout, sigmaFout, spdFout, aggFout, aggType, startYear,
             startDay, numDays);
