...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    ...
    printf(" numLinesToSkip is number of initial lines in file to ignore "
           "(default: 0)\n");
  ...
  // write numPerLine to file
  fwrite(&numPerLine, sizeof(int), 1, out);
  ...
  while (fscanf(in, "%f", &oneFloat) > 0) {  // while there are more floats
    numRead++;
    // write oneFloat to file
    fwrite(&oneFloat, sizeof(float), 1, out);
  }
