/* transpose: A stand-alone program
   ...
   Given a file that contains a rectangular matrix, replace the file with its
   transpose
...
/* Read file f into array of lines
   PRE: f is open for reading and rewound
        Each lines[i] is a character array of size maxLength, and lines
        contains at least nl such character arrays, where nl is the number of
        lines in the file
...
/* Read individual items from lines into items
   items[i][j] will hold a pointer to the jth item (j = 0..numItems-1) in the
   ith line (i = 0..numLines-1)
    - items are separated by SEPARATORS
...
/* Write out the transposed items to f
   ...
   items[i][j] holds the jth item (j = 0..numItems-1) on the ith line
   (i = 0..numLines-1)

   We output all items #0 on the first line, all items #1 on the second line,
   etc. Items are separated by spaces in the output file.
 */
...
int main(int argc, char *argv[]) {
  ...
  char ***items; /* 2-d array of character pointers, holding pointers to
                    various points in the lines array items[i][j] holds a
                    pointer to the jth item in the ith line */
  int i;
