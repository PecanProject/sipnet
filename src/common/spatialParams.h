...
typedef struct OneSpatialParamStruct {
  ...
  double *externalLoc;  // a pointer to a (non-spatial) copy of this parameter
                        // value elsewhere (e.g. local to the model)
  ...
  double *best;  // best value found so far; possibly patial; if non-changeable,
                 // ptr will be void
  double *knob; /* a "knob" that can be twisted to adjust this parameter's
       optimization (can be used however the individual optimization scheme
       wants) possibly spatial; if non-changeable, ptr will be void */

} OneSpatialParam;

typedef struct SpatialParamsStruct {
  OneSpatialParam *parameters;  // vector of parameters, dynamically-allocated
                                // (stored in order in which they were
                                // initialized)
  int maxParameters;  // maximum number of parameters
  int numParameters;  // actual number of parameters (tracks # that have been
  ...
  int *changeableParamIndices;  // indices (in parameters vector) of the
                                // changeable params (length of this vector is
                                // numChangeableParams)
...
/* Read all parameters from file, setting all values in spatialParams structure
   ...
   where
   value is a number if parameter is non-spatial, or * if parameter is spatial;
   changeable is 0 or 1 (boolean)

   If parameter is spatial, look in spatialParamFile for value, which has the
   following structure: name  value [value  value...] where the number of values
   is equal to the number of spatial locations
...
/* Return value of parameter i, location loc (if this parameter is
   non-spatial, ignore loc)
   PRE: 0 <= i < spatialParams->numParameters
...
/* Set best value of parameter i, location loc to have given value
   ...
   value
   PRE: 0 <= i < spatialParams->numParameters if this parameter is spatial,
   then -1 <= loc < spatialParams->numLocs
*/
...
/* Set best value of all parameters at given location to be equal to their
   current value (Note: ignores parameters that were never read in)
   If loc = -1, set best value of all parameters at ALL locations to be equal
...
/* If randomReset = 0, set values of all parameters to be equal to guess values
   If randomReset non-zero, set parameter values to be somewhere (chosen
   uniform randomly) between min and max
...
/* Write best parameter values, and other parameter info, to files
    (only write parameters that were read in, and in the same order that they
     were read in)
   Note that other parameter info won't have changed since the read - we just
   copy that over
   Write all parameter info to file with name *paramFile, and spatially-varying
   parameter values to file with name *spatialParamFile
...
/* Write name, guess, min & max, value, best and knob of each changeable param
   to file If loc >= 0, write info for that location; if loc = -1, write value
   at location 0, and append a * to spatial parameter values
