...
// maximum length of a parameter name, including the trailing '\0'
#define NAMELIST_INPUT_MAXNAME 64

// define named constants for the different possible input types
enum INPUT_TYPES { INT_TYPE, LONG_TYPE, DOUBLE_TYPE, STRING_TYPE };

// structure to hold a namelist input item, and a pointer to the next (for a
// linked list)
typedef struct NamelistInputItemStruct {
  char name[NAMELIST_INPUT_MAXNAME];  // name of parameter (must match name
                                      // given in input file)
  ...
  int maxlen; /* for STRING_TYPE, holds the maximum string length (including the
     trailing '\0'); ignored for other types */
