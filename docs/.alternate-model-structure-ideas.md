...
Not currently implemented. Mechanistically similar to fertilization, but should be part of met.
...
\begin{equation}
\mathbb{s}_{\text{C,stress},t}
=
1 -
\min\left(
1,
\frac{\mathbb{C}_{\text{NSC},t}}
{\mathbb{C}_{\text{NSC,ref},t}}
\right)
\end{equation}

where $\mathbb{C}_{\text{NSC},t}$ is an explicit non-structural carbon pool or a documented proxy, and $\mathbb{C}_{\text{NSC,ref},t}$ is a reference storage capacity such as a fraction of live structural biomass. 

Mortality would only occur after sustained stress:

\begin{equation}
\mathbb{I}_{\text{mort},t}
=
1
\quad \text{if} \quad
\overline{\mathbb{s}}_{\text{C,stress},t} >
\mathbb{s}_{\text{mort}}
\quad \text{for at least } \mathbb{\tau}_{\text{mort}} \text{ days}
\end{equation}

If mortality is triggered, live biomass should be routed mass-conservingly to litter, soil, or export pools:

\begin{equation}
\mathbb{F}^C_{\text{mort},j,t}
=
\mathbb{m}_{j,t} C_{j,t}
\end{equation}

where $j$ indexes live biomass pools and $0 \le \mathbb{m}_{j,t} \le 1$. Nitrogen associated with mortality fluxes should follow the C:N ratio of the source pool. Mortality should not silently delete carbon or nitrogen.
