...
size approach. This approach also has the advantage of accounting for reduced $N_2O$ flux when N is limiting (Zahele 
and Dalmorech 2011).

A new parameter $K_\text{vol}$ represents the first-order rate constant governing volatilization losses from the soil 
mineral nitrogen pool. The realized volatilization flux is proportional to $N_\text{min}$ and depends on temperature 
\eqref{eq:Braswell_A18b} and soil moisture \eqref{eq:water_nvol}.
