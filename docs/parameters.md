...
| $E_{\text{int,max}}/LAI$     | leafPoolDepth   | Cap on interception-evaporation flux per unit leaf area                                                               | $\text{cm} \cdot \text{day}^{-1}$                                              | Only used when `LEAF_WATER = 1` (`--leaf-water`). In `calcPrecip()`, defines an LAI-scaled cap on interception evaporation: $immedEvap \le LAI \cdot leafPoolDepth$. Excess throughfall goes to the soil. |
