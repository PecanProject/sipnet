...
| $f_{\text{fine root, exudation}}$ | fineRootExudation     | Fraction of $GPP$ from fine roots exuded to soil[^exudates]     | unitless | Planned extension; not currently a runtime param |
| $f_{\text{coarse root, exudation}}$ | coarseRootExudation   | Fraction of $GPP$ from coarse roots exuded to soil[^exudates]   | unitless | Planned extension; not currently a runtime param |
...
| $f_{\textrm{till}}$ | tillageEff          | Effect of tillage on decomposition that exponentially decays over time              | fraction                                             | Documented in model structure; event-level term in `events.in`; not currently a runtime param |

### Nitrogen Cycle Parameters

Current run-time parameters support mineralization, volatilization, leaching, and
pool stoichiometry.
...
| $f_{\text{fix,max}}$ | nFixFracMax         | Maximum fraction of plant N demand met by biological N fixation under low soil N            | fraction           | Documented in model structure; not currently a runtime param |
| $K_N$                | nFixHalfSatMinN     | Mineral N level at which fixation suppression factor $D_{N_\text{min}}$ equals 0.5          | $\text{g N} \cdot \text{m}^{-2}$ | Documented in model structure; not currently a runtime param |
...
| $f_{\text{drain}}$       | waterDrainFrac  | fraction of soil water above capacity removed by drainage per day                                                     | unitless                                                 | Documented in model structure; not currently a runtime param                                                                                  |
...
| $p$                     | anaerobicTransExp | Methane anoxia sensitivity exponent in $D_{\text{water},CH_4}=A^p$                                                 | unitless                                                 | Described in model structure; currently optional/not required in runtime parameter parsing                                                   |
