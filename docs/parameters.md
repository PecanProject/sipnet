...
|     | $C_{\text{wood,storage}}$  |                 | Wood carbon storage pool (delta)                                         | $\text{g C} \cdot \text{m}^{-2} \text{ ground area}$ | Tracks lag between NPP input and allocation output due to 5-day averaging; initialized to 0; can be negative. Total wood C = $C_{\text{wood}} + C_{\text{wood,storage}}$ |
