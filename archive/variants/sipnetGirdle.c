...
typedef struct GirdlingVars {
	int year;	// year that girdling occurs
	int day;	// day that girdling begins
} Girdling;
...
static Girdling girdling;


void readGirdlingData(char *girdleFile ) {
	FILE *in;
	
	
	in = openFile(girdleFile,"r");
	fscanf(in, "%d %d", &girdling.year, &girdling.day);
...
void soilDegradation() {
	...
	if ( (climate->day >= girdling.day) & (climate->year >= girdling.year )) {
		params.fineRootExudation = 0;
		params.coarseRootExudation = 0;
		
		#if SOIL_MULTIPOOL
			envi.soil[NUMBER_SOIL_CARBON_POOLS-1] += envi.fineRootC + envi.coarseRootC;
		#else
			envi.soil += envi.fineRootC + envi.coarseRootC;
		#endif
		envi.fineRootC = 0;
		envi.coarseRootC = 0;			// Make the allocation non zero in calculate fluxes
...
*/
int initModel(SpatialParams **spatialParams, int **steps, char *paramFile, char *climFile, char *girdlingFile) {
