...
typedef struct ModelOptionsStruct {  // options that are read in at fileName . options
							// in optionsSet we deal with the mutually exclusive parms
...
static ModelOptions options;	// set up model options
...
int * readClimData(char *climFile, int numLocs) {
  ...
  double thisGdd = 0.0; // growing degree days of this time step
...
void readOptions(char *optionsFile ) {		// read in options file
	// currently we have it reading LINE for LINE what is in the options file, and no
	// checking to make sure we have done it correctly.
	
	FILE *in;
	
	
	in = openFile(optionsFile,"r");
	
	fscanf(in, "%*s %d", &options.CSV_OUTPUT);
	fscanf(in, "%*s %d", &options.ALTERNATIVE_TRANS);
	fscanf(in, "%*s %d", &options.BALL_BERRY);
	fscanf(in, "%*s %d", &options.PENMAN_MONTEITH_TRANS);
	fscanf(in, "%*s %d", &options.GROWTH_RESP);
	fscanf(in, "%*s %d", &options.LLOYD_TAYLOR);
	fscanf(in, "%*s %d", &options.SEASONAL_R_SOIL);
	fscanf(in, "%*s %d", &options.WATER_PSN);
	fscanf(in, "%*s %d", &options.WATER_HRESP);
	fscanf(in, "%*s %d", &options.DAYCENT_WATER_HRESP);
	
	fscanf(in, "%*s %d", &options.MODEL_WATER);										
	fscanf(in, "%*s %d", &options.COMPLEX_WATER);	
	fscanf(in, "%*s %d", &options.LITTER_WATER);	
	fscanf(in, "%*s %d", &options.LITTER_WATER_DRAINAGE);	
	fscanf(in, "%*s %d", &options.SNOW);			
	fscanf(in, "%*s %d", &options.GDD);										
	fscanf(in, "%*s %d", &options.SOIL_PHENOL);	
	fscanf(in, "%*s %d", &options.LITTER_POOL);	
	fscanf(in, "%*s %d", &options.SOIL_MULTIPOOL);	
	fscanf(in, "%*s %d", &options.NUMBER_SOIL_CARBON_POOLS);
		
	fscanf(in, "%*s %d", &options.SOIL_QUALITY);	
	fscanf(in, "%*s %d", &options.MICROBES);
	fscanf(in, "%*s %d", &options.STOICHIOMETRY);	
	fscanf(in, "%*s %d", &options.ROOTS);	
...
void outputState(FILE *out, int loc, int year, int day, double time) {
  		...
  		if (options.SOIL_MULTIPOOL) {
  			int counter;
  	
  			for(counter=0; counter<options.NUMBER_SOIL_CARBON_POOLS; counter++) {
...
if (options.WATER_PSN) {// we're modeling water stress
...
if (options.SNOW) { // we're modeling snow
...
if (options.LITTER_WATER_DRAINAGE) { // we're calculating drainage even when evap. layer is not overflowing
...
void calcMaintenanceRespiration(double tsoil, double water, double whc) {
	...
	if (options.WATER_HRESP) { // if soil moisture affects heterotrophic resp.

		if (options.DAYCENT_WATER_HRESP) { // using DAYCENT formulation
	...
	if (options.SOIL_MULTIPOOL) {
		...
		for(counter=0;counter<options.NUMBER_SOIL_CARBON_POOLS;counter++) {			// Loop through all the soil carbon pools


			if (options.SOIL_QUALITY)
				soilQuality=(counter+1)/options.NUMBER_SOIL_CARBON_POOLS;	// Ensures that the quality will never be zero
			else
				soilQuality=0;
			...
			if (options.SEASONAL_R_SOIL) { 		// decide which parameters to use based on tsoil
		...
		if (options.MICROBES) {	// If we don't have a multipool approach, respiration is determined by microbe biomass	 					
...
void microbeGrowth() {
	if (options.SOIL_MULTIPOOL) {
		...
		for( counter=0; counter < options.NUMBER_SOIL_CARBON_POOLS; counter++) {
			...
			}  //We need code in here if we just have a rate coefficient pool	
		}
	}
	else {
		if (options.MICROBES) {
...
int litterInputPool(double qualityComponent) {
	return floor(qualityComponent * options.NUMBER_SOIL_CARBON_POOLS);	// Ensures that the input pool will never be 0
}


void soilDegradation() {
		int counter;	// Counter of different soil pools
		int soilPoolsMax = &options.NUMBER_SOIL_CARBON_POOLS;	
	double soilWater;
	if (options.MODEL_WATER) // take soilWater from environment
	...
	if (options.SOIL_MULTIPOOL) {
		...
		if (options.SOIL_QUALITY) {	
			...
			for( counter=soilPoolsMax-1; -1 < counter; counter--) {
				litterInput=0;		// Initialize the total litter input with every loop
			
			
				soilQuality=(counter+1)/soilPoolsMax;
			...
			envi.soil[soilPoolsMax-1]+=(fluxes.coarseRootLoss+fluxes.fineRootLoss) * climate->length;
	  ...
	  else if (options.LITTER_POOL) {		// If LITTER_POOL = 1, then all other bets are off
...
void calculateFluxes() {
  // auxiliary variables:
  	...
  	double litterWater, soilWater; /* amount of water in litter and soil (cm)
  		...
  		double growthResp=0.0; // g C * m^-2 ground area * day^-1
	    double netRain=0.0;		// COMPLEX_WATER options  rain - immedEvap (cm/day) 


	


	if (options.MODEL_WATER) {// take litterWater and soilWater from environment
	...
	if (options.MODEL_WATER) { // water modeling happens here:

		if (options.COMPLEX_WATER) {
...
void updateState() {
	...
	if (options.MODEL_WATER) { // water pool updating happens here:

		if (options.LITTER_WATER) { // (2 soil water layers; litter water will only be on if complex water is also on)
...
void optionsSet() {		// Set all the options to be the correct values for the compound values

options.SEASONAL_R_SOIL=options.SEASONAL_R_SOIL && !options.LLOYD_TAYLOR;

options.DAYCENT_WATER_HRESP=options.DAYCENT_WATER_HRESP && options.WATER_HRESP;

options.COMPLEX_WATER=options.COMPLEX_WATER && options.MODEL_WATER;

options.LITTER_WATER = options.LITTER_WATER && options.COMPLEX_WATER;
options.LITTER_WATER_DRAINAGE =options.LITTER_WATER_DRAINAGE && options.LITTER_WATER;

options.SNOW=(options.SNOW || options.COMPLEX_WATER) && options.MODEL_WATER;

options.SOIL_PHENOL = options.SOIL_PHENOL && !options.GDD;

options.SOIL_MULTIPOOL=options.SOIL_MULTIPOOL && !options.LITTER_POOL;

options.SOIL_QUALITY=options.SOIL_QUALITY && options.SOIL_MULTIPOOL;

options.MICROBES=options.MICROBES && !options.SOIL_MULTIPOOL;
options.STOICHIOMETRY=options.STOICHIOMETRY && options.MICROBES; 
...
// If SOIL_QUALITY, split initial soilCarbon equally among all the pools
 	if (options.SOIL_MULTIPOOL) {
		...
		for( counter=0; counter <options.NUMBER_SOIL_CARBON_POOLS; counter++) {
			...
			envi.soil[counter]=params.soilInit/options.NUMBER_SOIL_CARBON_POOLS; 
