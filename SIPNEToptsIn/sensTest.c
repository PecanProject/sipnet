...
void usage(char *progName) {
  ...
  printf("Usage: %s fileName paramNum minVal maxVal numRuns typeNum1 [typeNum2 ...], where:\n", progName);
  ...
  printf("parameter is varied from minVal to maxVal over a total of numRuns\n\n");
...
int main(int argc, char *argv[]) {
  ...
  char optionsFile[256]; // name of options file
  ...
  if (argc < (optind + 6)) { // expecting at least 6 more arguments, but there are fewer than that
    usage(argv[0]);
    exit(1);
  }
  ...
  strcpy(optionsFile, fileName);
  strcat(optionsFile, ".options");
  
  strcpy(outFileName, fileName);
  strcat(outFileName, ".senstest");

  paramNum = strtol(argv[optind+1], &errc, 0);
  minVal = strtod(argv[optind+2], &errc);
  maxVal = strtod(argv[optind+3], &errc);
  numRuns = strtol(argv[optind+4], &errc, 0);
  numDataTypes = parseDataTypeIndices(dataTypeIndices, argv, optind+5, argc);
