/* runmean: structure and functions to keep track of a running weighted mean
   ...
   Each value has an associated weight, and mean is computed over a given total weight
...
int addValueToMeanTracker(MeanTracker *tracker, double value, double weight) {
  ...
  if (weight <= 0) // error
  ...
  else if (weight >= tracker->totWeight) // current value knocks out all previous values
    resetMeanTracker(tracker, value);
  else { // 0 < weight < totWeight: remove oldest values to make room for new (keeping total weight the same)
    ...
    while (weightLeft > 0) { // while we still have to subtract more
      if (tracker->weights[i] > weightLeft) { // subtract a portion of this one and we'll be done 
	tracker->weights[i] -= weightLeft; // subtract correct portion from current weight
	tracker->sum -= weightLeft * tracker->values[i]; // subtract correct portion from sum
	...
	weightLeft -= tracker->weights[i]; // we now have this much less weight to subtract
	i = (i + 1) % tracker->length; // forget this value
	// note: we don't have to worry about going past last value, since 1) we check for weight >= tracker->totWeight,
	// and 2) before starting this function, we know that sum(tracker->weights[start..last] == tracker->totWeight)
      }
    } // end while: now weightLeft = 0
    
    tracker->start = i; // move start position along to correct new position
    i = (tracker->last + 1) % tracker->length; // point to position of new value
    if (i == tracker->start) { // uh-oh: we've wrapped around and are out of space!
      // restore previous state and return error value
      // note: this will only happen if we only executed the while loop once, and entered the first if block
      // we can take advantage of this fact to restore the old state
