...
#define THRESH 0.02  // how close we have to get to A_STAR before stop adjusting
// temperatures

// write a header line for the .hist file
void writeHistFileHeader(FILE *histFile, int numYears, int np,
                         int numDataTypes) {
  fprintf(histFile,
          "Log_likelihood  %dx(Sigma_estimate  Mean_error  "
          "Daily_aggregated_mean_error  %dxAnnual_Totals)  %dxParam_values\n",
          numDataTypes, numYears, np);
}

// write one point to the .hist file for the given location (loc)
void writeHistFile(FILE *histFile, double ltotnew, double *sigma,
                   OutputInfo *outputInfo, int numDataTypes,
...
// write a header for the .hist file in binary format (a single int giving
// number of floats per point)
void writeHistFileBinHeader(FILE *histFile, int numYears, int np,
                            int numDataTypes) {
  int numPerPoint;

  numPerPoint = 1 + numDataTypes * (3 + numYears) + np;
  // for each point, we write ltot plus, for each data type, sigma, mean error,
  // days error, yearly sums, plus each param. value
...
// (writing it as a float rather than a double saves space, at the expense of a
// little precision)
...
// write one point to the .hist file for the given location (loc), in binary
// format
void writeHistFileBin(FILE *histFile, double ltotnew, double *sigma,
                      OutputInfo *outputInfo, int numDataTypes,
                      SpatialParams *spatialParams, int loc) {
  int i, j;
  int np;  // number of changeable parameters
...
// parameters from spatialParams through outputInfo are outputs, rest are inputs
// (spatialParams also provides some inputs) all output parameters are arrays
// except ltotnew, ltotold and ltotmax; loglikely, outputInfo and sigma are
// spatial (1st dimension of sigma and outputInfo is spatial dimension;
// loglikely is only one-dimensional)
//
// if loc = -1, we're running everywhere (number of locations, which is also the
// size of loglikely and sigma, specified in spatialParams)
//
// if loc >= 0, we're running at a single location; in this case, 1st dimension
// of loglikely and sigma only has length 1
//
// randomStart is boolean: do we start each chain with a random param. set (as
// opposed to guess values)?
void reset(
    SpatialParams *spatialParams, double *loglikely, double *ltotnew,
...
/* write (to chainInfo file) all information needed to restart a chain from a
   given point format of file: binary file as follows: ltotold ltotmax  <np> *
   (value best knob) where np is the number of changeable parameters

   if loc = -1, then for each spatial parameter we write all values (i.e. value
   at each point) followed by all bests and all knobs

   if loc >= 0, then we only write value, best and knob at the given location
   where all values are doubles
*/
void writeChainInfo(char *chainInfo, double ltotold, double ltotmax,
                    SpatialParams *spatialParams, int loc) {
  FILE *out;
  int np;  // number of changeable parameters
  int i, index;
  ...
  for (i = 0; i < np; i++) {
    index = spatialParams->changeableParamIndices[i];  // get index of ith
    ...
    for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
      ...
      value[0] = getSpatialParam(spatialParams, index, currLoc);
      fwrite(value, sizeof(double), 1, out);
    }
    for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
      // if non-spatial, or loc != -1, we'll only execute this loop once
      value[0] = getSpatialParamBest(spatialParams, index, currLoc);
      fwrite(value, sizeof(double), 1, out);
    }
    for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
      // if non-spatial, or loc != -1, we'll only execute this loop once
      value[0] = getSpatialParamKnob(spatialParams, index, currLoc);
      fwrite(value, sizeof(double), 1, out);
    }
  }
...
// read (from chainInfo file) all information needed to restart a chain from a
// given point (see writeChainInfo function for format of file) ltotold and
// ltotmax are pointers (NOT vectors)
void readChainInfo(char *chainInfo, double *ltotold, double *ltotmax,
                   SpatialParams *spatialParams, int loc) {
  FILE *in;
  int np;  // number of changeable parameters
  int i, index;
...
/* Puts best parameters found in spatialParams
   ...
   location. randomStart is boolean: do we start each chain with a random param.
   set (as opposed to guess values)? NOTE: anything but a scale factor of 1 goes
   against theory */
void metropolis(
  ...
  // we're actually running at (based on both loc and spatialParams->numLocs)
  int thisFirstLoc, thisLastLoc;  // for spatial params, same as firstLoc and
  // lastLoc; for non-spatial params, both are 0
  ...
  double *pold;  // spatial
  double pdelta, padd;
  double **sigma;  // sigma of this point, estimated by likely function (2-d
                   // array: sigma[i][j] is sigma at loc. i, data type j)
  OutputInfo **outputInfo;  // store error info and annual totals for each run
  // & each data type 2-d array: outputInfo[i][j] is outputInfo at location i,
  // data type j
  int converged = 0; /* have we converged on correct param ranges yet?
      we've converged when we're near A_STAR acceptance
      If numChains > 1, converged will only be set to 1 after we've converged
      (numChains) times */
  long totalIters = numSpinUps + estSteps;  // how many total steps to take once
  ...
  fprintf(userOut, "\n\nRESETTING FOR START CHAIN %d of %d\n\n", chainNum,
          numChains);
  reset(spatialParams, loglikely, &ltotnew, &ltotold, &ltotmax, sigma,
        outputInfo, loc, randomStart, addFraction, userOut, likely, paramWeight,
        model, dataTypeIndices, numDataTypes, costFunction, dataTypeWeights);

  writeChangeableParamInfo(spatialParams, loc, userOut);

  bestChainLtotmax = -1.0 * DBL_MAX;  // really friggin' small (i.e. really
                                      // friggin' big in the negative direction)
  k = 1;

  // open all hist files (one for each location), assign file pointers
  // (histFiles), write header to each
  for (currLoc = firstLoc; currLoc <= lastLoc; currLoc++) {
    locIndex = currLoc - firstLoc;  // index into array
    // append currLoc to end of histFileBase to get name of current file
    sprintf(histFileName, "%s%d", histFileBase, currLoc);
    histFiles[locIndex] = openFile(histFileName, "w");

    // writeHistFileHeader(histFiles[locIndex],
    // outputInfo[locIndex][0].numYears, spatialParams->numChangeableParams,
    // numDataTypes);
    writeHistFileBinHeader(histFiles[locIndex],
                           outputInfo[locIndex][0].numYears,
                           spatialParams->numChangeableParams, numDataTypes);
    /* NOTE: 1) this has to be done AFTER call to reset, since
       outputInfo.numYears is set in call to likely function

       2) it doesn't matter which data type we use for outputInfo (here we use
       0), since all will have the same numYears */
  }

  /*****metropolis loop***********************************************/

  while ((converged == 0) || (k <= totalIters)) {
    accept = 1;  // so far, we're in accept mode - we haven't rejected the new
                 // point yet

    /*select parameter to change at random*/
    ichg = randomChangeableSpatialParam(spatialParams);

    /* determine first and last location for THIS parameter (depends on whether
       parameter is spatial) note that we'll still use firstLoc and lastLoc for
       things like actually running model; thisFirstLoc and thisLastLoc just
       refer to locations we care about for parameter-related things like
       choosing a new parameter value */
    ...
    range = (getSpatialParamMax(spatialParams, ichg) -
             getSpatialParamMin(spatialParams, ichg));  // range is the same for
                                                        // all locations
    for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
      oldVal = getSpatialParam(spatialParams, ichg, currLoc);
      pold[currLoc - thisFirstLoc] = oldVal;  // remember old value
      if (accept == 1) {
        // we haven't set accept to 0 yet (if accept = 0 already, don't bother
        // setting new param. values)

        // pdelta is expressed as fraction of parameter's range
        ...
        if (checkSpatialParam(spatialParams, ichg, oldVal + padd) ==
            0) {  // outside allowable range
          accept = 0; /* if new value is outside allowable range at any
    ...
    if (accept == 1) {  // we're within allowable range at all locations; run
      ...
      if (ltotnew > ltotmax) {
        ltotmax = ltotnew;
        setAllSpatialParamBests(spatialParams, loc); /* set best equal to
                    current value for all parameters if loc == -1, this will set
    ...
    if (accept == 1) {
      ...
      if (converged == 0) {  // we haven't converged yet - twist knob
                             // (equivalent to old pdelta)
        for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
          pdelta = getSpatialParamKnob(spatialParams, ichg, currLoc);
          pdelta = pdelta * INC;  // increase temperature
          // we used to prevent temperature from going above 1, but we no longer
          // care about how big pdelta gets
          setSpatialParamKnob(spatialParams, ichg, currLoc, pdelta);
        }
      } else if (k > numSpinUps) {  // only write to history files if we have
                                    // been converged for > numSpinUps steps
        /* NOTE: I THINK WE SHOULD TECHNICALLY BE WRITING TO HIST FILE WHETHER
           WE ACCEPT OR REJECT IF WE REJECT, SHOULD RE-WRITE OLD POINT TO HIST
           FILE (WILL HAVE TO SAVE OLD LOGLIKELY, SIGMA, AND OUTPUTINFO) TO DO
           THIS, REMOVE THIS ELSE IF BLOCK, AND PUT IT IN AN IF (CONVERGED && K
           > NUMSPINUPS) BLOCK AFTER END OF ELSE (REJECTION) BLOCK */
        for (currLoc = firstLoc; currLoc <= lastLoc; currLoc++) {
          locIndex = currLoc - firstLoc;
          // writeHistFile(histFiles[locIndex], loglikely[locIndex],
          // sigma[locIndex], outputInfo[locIndex], numDataTypes, spatialParams,
          // currLoc);
          writeHistFileBin(histFiles[locIndex], loglikely[locIndex],
                           sigma[locIndex], outputInfo[locIndex], numDataTypes,
                           spatialParams, currLoc);
        }
      }
    }
    ...
    /* act on rejection */
      ...
      for (currLoc = thisFirstLoc; currLoc <= thisLastLoc; currLoc++) {
        ...
        setSpatialParam(spatialParams, ichg, currLoc, oldVal);  // restore old
                                                                // value
      }
    ...
    if (k % numAtOnce == 0) {
      ...
      if (converged == 0) {  // we haven't converged yet - check for
                             // convergence:
        if (fabs(yes * 1.0 / numAtOnce - A_STAR) < THRESH) {  // we've
                                                              // newly-converged
          if (ltotmax >= bestChainLtotmax) {  // this is the best chain we've
                                              // found so far
            ...
            fprintf(userOut, "\n\nBEST START CHAIN FOUND SO FAR: WRITING CHAIN "
                             "INFO TO FILE\n\n");
          }
          ...
          if (chainNum < numChains) {  // reset for the next chain
            ...
            fprintf(userOut, "\n\nRESETTING FOR START CHAIN %d of %d\n\n",
                    chainNum, numChains);
            reset(spatialParams, loglikely, &ltotnew, &ltotold, &ltotmax, sigma,
                  outputInfo, loc, randomStart, addFraction, userOut, likely,
                  paramWeight, model, dataTypeIndices, numDataTypes,
                  costFunction, dataTypeWeights);
          }

          else {  // chainNum >= numChains: we're done running start chains,
                  // time to read best from file
            converged = 1;
            fprintf(userOut, "\n\nCONVERGED\n\n");

            if (ltotmax < bestChainLtotmax) {  // the most recent chain wasn't
                                               // the best: read best from file
  ...
  /* NOTE: may want to print (to file) some measure of best point here
     (e.g. ltotmax; may even want to print outputInfo of best point, which would
     be more easily done in metropolis loop, re-writing best file each time we
     find a new best point)
  */
