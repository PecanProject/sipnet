...
/**
 * @brief Compute light absorbed using simple integration
 * 
 * **CURRENTLY UNUSED** (see calcLightEff3)
 * canopy light effect (simple integration).
 *
 * This uses light attenuation (as in PnET) to first calculate light intensity and then the light effect `lightEFF`
 * for each layer.
 * 
 ...
 *
 * Like calcLightEff3 but here we use Simpson's method to approximate the integral.
 * 
...
int litterInputPool(double qualityComponent) {
	return (int) floor(qualityComponent * NUMBER_SOIL_CARBON_POOLS);	// Ensures that the input pool will never be 0
}
...
// If SOIL_QUALITY, split initial soilCarbon equally among all the pools
  ...
  resetMeanTracker(meanFPAR, 0); // initialize with mean FPAR (over last MEAN_FPAR_DAYS) of 0
}
...
#if EXTRA_DATA_TYPES
  static char *DATA_TYPES[MAX_DATA_TYPES] = {"EVAPOTRANSPIRATION", "NEE", "SOIL_WETNESS","FPAR"
					     "GPP", "RTOT", "RA", "RH", "NPP",
					     "YEARLY_GPP", "YEARLY_RTOT", "YEARLY_RA", "YEARLY_RH", "YEARLY_NPP", "YEARLY_NEE",
					     "TOT_GPP", "TOT_RTOT", "TOT_RA", "TOT_RH", "TOT_NPP", "TOT_NEE"};
#else
  static char *DATA_TYPES[MAX_DATA_TYPES] = {"EVAPOTRANSPIRATION", "NEE", "SOIL_WETNESS","FAPAR","YEARLY_NEE"};
#endif
...
/* char **getDataTypeNames() {
	...
	static char *DATA_TYPES[MAX_DATA_TYPES] = {"EVAPOTRANSPIRATION", "NEE", "SOIL_WETNESS",
		"GPP", "R_SOIL", "ANNUAL_LITTER", "LAI", "PLANT_WOOD_C","RTOT", "RA", "RH", "NPP",
		"YEARLY_GPP", "YEARLY_RTOT", "YEARLY_RA", "YEARLY_RH", "YEARLY_NPP", "YEARLY_NEE",
		"TOT_GPP", "TOT_RTOT", "TOT_RA", "TOT_RH", "TOT_NPP", "TOT_NEE"};
#else
