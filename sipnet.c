...
int * readClimData(char *climFile, int numLocs) {
	...
	while (status != EOF) {
		...
		status = fscanf(in,
				"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &loc,
		...
		if (status != EOF) { // we have another climate record
			...
			if (loc == currLoc) { // still reading climate records from the same place: add a node at end of linked list
				...
				printf(
						"Error reading climate file: trying to read location %d, but numLocs = %d\n",
						loc, numLocs);
				exit(1);
				...
				firstClimates[currLoc]
						= (ClimateNode *)malloc(sizeof(ClimateNode)); // allocate space for head
				curr = next = firstClimates[currLoc]; // we'll start writing to next linked list 
			} else { // loc < currLoc
				printf(
						"Error reading climate file: was reading location %d, trying to read location %d\n",
						currLoc, loc);
...
int readParamData(SpatialParams **spatialParamsPtr, char *paramFile,
	...
	*spatialParamsPtr = newSpatialParams(NUM_PARAMS, numLocs);
	...
	initializeOneSpatialParam(spatialParams, "plantWoodInit",
			&(params.plantWoodInit), 1);
	initializeOneSpatialParam(spatialParams, "laiInit", &(params.laiInit), 1);
	initializeOneSpatialParam(spatialParams, "litterInit",
			&(params.litterInit), 1);
	initializeOneSpatialParam(spatialParams, "soilInit", &(params.soilInit), 1);
	initializeOneSpatialParam(spatialParams, "litterWFracInit",
			&(params.litterWFracInit), 1);
	initializeOneSpatialParam(spatialParams, "soilWFracInit",
			&(params.soilWFracInit), 1);
	...
	initializeOneSpatialParam(spatialParams, "baseFolRespFrac",
			&(params.baseFolRespFrac), 1);
	...
	initializeOneSpatialParam(spatialParams, "vegRespQ10",
			&(params.vegRespQ10), 1);
	initializeOneSpatialParam(spatialParams, "growthRespFrac",
			&(params.growthRespFrac), GROWTH_RESP);
	initializeOneSpatialParam(spatialParams, "frozenSoilFolREff",
			&(params.frozenSoilFolREff), 1);
	initializeOneSpatialParam(spatialParams, "frozenSoilThreshold",
			&(params.frozenSoilThreshold), 1);
	initializeOneSpatialParam(spatialParams, "dVpdSlope", &(params.dVpdSlope),
			1);
	initializeOneSpatialParam(spatialParams, "dVpdExp", &(params.dVpdExp), 1);
	initializeOneSpatialParam(spatialParams, "halfSatPar",
			&(params.halfSatPar), 1);
	initializeOneSpatialParam(spatialParams, "attenuation",
			&(params.attenuation), 1);

	initializeOneSpatialParam(spatialParams, "leafOnDay", &(params.leafOnDay),
			!((GDD) || (SOIL_PHENOL)));
	initializeOneSpatialParam(spatialParams, "gddLeafOn", &(params.gddLeafOn),
			GDD);
	initializeOneSpatialParam(spatialParams, "soilTempLeafOn",
			&(params.soilTempLeafOn), SOIL_PHENOL);
	initializeOneSpatialParam(spatialParams, "leafOffDay",
			&(params.leafOffDay), 1);
	initializeOneSpatialParam(spatialParams, "leafGrowth",
			&(params.leafGrowth), 1);
	initializeOneSpatialParam(spatialParams, "fracLeafFall",
			&(params.fracLeafFall), 1);
	initializeOneSpatialParam(spatialParams, "leafAllocation",
			&(params.leafAllocation), 1);
	initializeOneSpatialParam(spatialParams, "leafTurnoverRate",
			&(params.leafTurnoverRate), 1);
	initializeOneSpatialParam(spatialParams, "baseVegResp",
			&(params.baseVegResp), 1);
	initializeOneSpatialParam(spatialParams, "litterBreakdownRate",
			&(params.litterBreakdownRate), LITTER_POOL);

	initializeOneSpatialParam(spatialParams, "fracLitterRespired",
			&(params.fracLitterRespired), LITTER_POOL);
	initializeOneSpatialParam(spatialParams, "baseSoilResp",
			&(params.baseSoilResp), 1);
	initializeOneSpatialParam(spatialParams, "baseSoilRespCold",
			&(params.baseSoilRespCold), SEASONAL_R_SOIL);
	initializeOneSpatialParam(spatialParams, "soilRespQ10",
			&(params.soilRespQ10), 1);
	initializeOneSpatialParam(spatialParams, "soilRespQ10Cold",
			&(params.soilRespQ10Cold), SEASONAL_R_SOIL);
	initializeOneSpatialParam(spatialParams, "coldSoilThreshold",
			&(params.coldSoilThreshold), SEASONAL_R_SOIL);
	...
	initializeOneSpatialParam(spatialParams, "soilRespMoistEffect",
			&(params.soilRespMoistEffect), ((WATER_HRESP)
					&& !(DAYCENT_WATER_HRESP)));
	initializeOneSpatialParam(spatialParams, "waterRemoveFrac",
			&(params.waterRemoveFrac), 1);
	initializeOneSpatialParam(spatialParams, "frozenSoilEff",
			&(params.frozenSoilEff), 1);
	initializeOneSpatialParam(spatialParams, "wueConst", &(params.wueConst), 1);
	initializeOneSpatialParam(spatialParams, "litterWHC", &(params.litterWHC),
			1);
	initializeOneSpatialParam(spatialParams, "soilWHC", &(params.soilWHC), 1);
	initializeOneSpatialParam(spatialParams, "immedEvapFrac",
			&(params.immedEvapFrac), COMPLEX_WATER);
	initializeOneSpatialParam(spatialParams, "fastFlowFrac",
			&(params.fastFlowFrac), COMPLEX_WATER);

	initializeOneSpatialParam(spatialParams, "snowMelt", &(params.snowMelt),
			SNOW);
	initializeOneSpatialParam(spatialParams, "litWaterDrainRate",
			&(params.litWaterDrainRate), LITTER_WATER_DRAINAGE);
	initializeOneSpatialParam(spatialParams, "rdConst", &(params.rdConst),
			(COMPLEX_WATER) || (PENMAN_MONTEITH_TRANS));
	initializeOneSpatialParam(spatialParams, "rSoilConst1",
			&(params.rSoilConst1), COMPLEX_WATER);
	initializeOneSpatialParam(spatialParams, "rSoilConst2",
			&(params.rSoilConst2), COMPLEX_WATER);
	initializeOneSpatialParam(spatialParams, "leafCSpWt", &(params.leafCSpWt),
			1);
	initializeOneSpatialParam(spatialParams, "cFracLeaf", &(params.cFracLeaf),
			1);
	initializeOneSpatialParam(spatialParams, "woodTurnoverRate",
			&(params.woodTurnoverRate), 1);
	initializeOneSpatialParam(spatialParams, "qualityLeaf",
			&(params.qualityLeaf), SOIL_QUALITY);
	initializeOneSpatialParam(spatialParams, "qualityWood",
			&(params.qualityWood), SOIL_QUALITY);

	initializeOneSpatialParam(spatialParams, "efficiency",
			&(params.efficiency), (SOIL_QUALITY) || (MICROBES));
	initializeOneSpatialParam(spatialParams, "maxIngestionRate",
			&(params.maxIngestionRate), (SOIL_QUALITY) || (MICROBES));
	initializeOneSpatialParam(spatialParams, "halfSatIngestion",
			&(params.halfSatIngestion), MICROBES);
	initializeOneSpatialParam(spatialParams, "totNitrogen",
			&(params.totNitrogen), STOICHIOMETRY);
	initializeOneSpatialParam(spatialParams, "microbeNC", &(params.microbeNC),
			STOICHIOMETRY);
	initializeOneSpatialParam(spatialParams, "microbeInit",
			&(params.microbeInit), (SOIL_QUALITY) || (MICROBES));
	initializeOneSpatialParam(spatialParams, "fineRootFrac",
			&(params.fineRootFrac), ROOTS);
	initializeOneSpatialParam(spatialParams, "coarseRootFrac",
			&(params.coarseRootFrac), ROOTS);

	initializeOneSpatialParam(spatialParams, "fineRootAllocation",
			&(params.fineRootAllocation), ROOTS);
	initializeOneSpatialParam(spatialParams, "woodAllocation",
			&(params.woodAllocation), ROOTS);
	initializeOneSpatialParam(spatialParams, "fineRootExudation",
			&(params.fineRootExudation), ROOTS);
	initializeOneSpatialParam(spatialParams, "coarseRootExudation",
			&(params.coarseRootExudation), ROOTS);
	initializeOneSpatialParam(spatialParams, "fineRootTurnoverRate",
			&(params.fineRootTurnoverRate), ROOTS);
	initializeOneSpatialParam(spatialParams, "coarseRootTurnoverRate",
			&(params.coarseRootTurnoverRate), ROOTS);
	initializeOneSpatialParam(spatialParams, "baseFineRootResp",
			&(params.baseFineRootResp), ROOTS);
	initializeOneSpatialParam(spatialParams, "baseCoarseRootResp",
			&(params.baseCoarseRootResp), ROOTS);
	initializeOneSpatialParam(spatialParams, "fineRootQ10",
			&(params.fineRootQ10), ROOTS);
	initializeOneSpatialParam(spatialParams, "coarseRootQ10",
			&(params.coarseRootQ10), ROOTS);

	initializeOneSpatialParam(spatialParams, "baseMicrobeResp",
			&(params.baseMicrobeResp), MICROBES);
	initializeOneSpatialParam(spatialParams, "microbeQ10",
			&(params.microbeQ10), MICROBES);
	initializeOneSpatialParam(spatialParams, "microbePulseEff",
			&(params.microbePulseEff), (ROOTS) && (MICROBES));
	initializeOneSpatialParam(spatialParams, "m_ballBerry",
			&(params.m_ballBerry), 0);
...
void outputHeader(FILE *out) {
	...
	fprintf(
			out,
			"evapotranspiration transpiration nee totNee gpp rAboveground rSoil rRoot rtot\n");
}
...
void outputState(FILE *out, int loc, int year, int day, double time) {
	...
	fprintf(out, " %8.2f %8.2f %8.2f %8.3f %8.2f ", envi.litter,
			envi.litterWater, envi.soilWater, trackers.soilWetnessFrac,
			envi.snow);
	fprintf(out, "%8.5f %8.5f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f\n",
			trackers.evapotranspiration, trackers.transpiration, trackers.nee,
			trackers.totNee, trackers.gpp, trackers.rAboveground,
...
void outputStatecsv(FILE *out, int loc, int year, int day, double time) {
	...
	fprintf(
			out,
			"%8.2f , %8.3f, %8.2f , %8.3f , %8.2f , %8.2f , %8.2f , %8.8f , %8.2f , %8.3f , %8.3f , %8.3f, %8.3f , %8.3f , %8.3f  \n",
			envi.litter, envi.litterWater, envi.soilWater,
			trackers.soilWetnessFrac, envi.snow, trackers.npp,
			trackers.yearlyNpp, trackers.nee, trackers.totNee,
			trackers.evapotranspiration, fluxes.transpiration, trackers.gpp,
			trackers.ra, trackers.rh, trackers.rtot);
}
...
void calcLightEff2(double *lightEff, double lai, double par) {
	...
	if (lai > 0 && par > 0) { // must have at least some leaves and some light
		for (layer = 1; layer <= NUM_LAYERS; layer++) {
			...
			cumLightEff += (1 - pow(2,
					(-1.0 * lightIntensity/params.halfSatPar))); // add between 0 and 1
...
void calcLightEff3(double *lightEff, double lai, double par) {
	...
	if (lai > 0 && par > 0) { // must have at least some leaves and some light
		...
		while (layer <= NUM_LAYERS) {
			cumLai = lai * ((double)layer / NUM_LAYERS); // lai from this layer up
			lightIntensity = par * exp(-1.0 * params.attenuation * cumLai); // between 0 and par
			currLightEff = (1 - pow(2,
					(-1.0 * lightIntensity/params.halfSatPar))); // between 0 and 1
...
void potPsn(double *potGrossPsn, double *baseFolResp, double lai, double tair,
	...
	dTemp = (params.psnTMax - tair)*(tair - params.psnTMin)/pow((params.psnTMax
			- params.psnTMin)/2.0, 2);
	if (dTemp < 0)
		dTemp = 0.0;
...
void moisture_bwb(double *trans, double *dWater, double potGrossPsn,
		double vpd, double vPress, double plantLeafC, double leafCSpWt,
		double soilWater) {
...
void moisture_pm(double *trans, double *dWater, double potGrossPsn, double vpd,
	...
	if (potGrossPsn < TINY) { // avoid divide by 0
		...
		*dWater = 1; // dWater doesn't matter, since we don't have any photosynthesis
		...
		potTrans = (DELTA*((1- gapfraction/100)*climate->par - climate->tsoil)
				+ (RHO*CP*vpd)/(rCanConst/climate->wspd))/(DELTA+GAMMA*(1 + (params.rdConst/climate->wspd)/(rCanConst/climate->wspd)));
...
void simpleWaterFlow(double *rain, double *snowFall, double *immedEvap,
		...
		double *evaporation, double *topDrainage, double *bottomDrainage,
		double water, double snow, double precip, double temp, double length,
		double trans) {
...
void snowPack(double *snowMelt, double *sublimation, double snowFall) {
	...
	static const double CONVERSION = (RHO * CP)/GAMMA * (1./LAMBDA_S) * 1000.
...
void evapSoilFluxes(double *fastFlow, double *evaporation, double *drainage,
	...
	*fastFlow = netIn * params.fastFlowFrac;
	...
	waterRemaining = water + netIn * climate->length - fluxesOut
			* climate->length;
...
void transSoilDrainage(double *bottomDrainage, double topDrainage,
...
void soilWaterFluxes(double *fastFlow, double *evaporation,
		double *topDrainage, double *bottomDrainage, double netRain,
		double snowMelt, double trans, double litterWater, double soilWater) {
...
void vegResp(double *folResp, double *woodResp, double baseFolResp) {
	*folResp = baseFolResp * pow(params.vegRespQ10, (climate->tair
			- params.psnTOpt)/10.0);
	...
	*woodResp = params.baseVegResp * envi.plantWoodC * pow(params.vegRespQ10,
			climate->tair/10.0);
}
...
void ensureAllocation() {
	...
	allocationSum = params.leafAllocation+params.woodAllocation
...
void soilDegradation() {
	...
	* climate->length;
	...
	envi.soil+=(fluxes.coarseRootLoss+fluxes.fineRootLoss+fluxes.woodLitter
			+ fluxes.leafLitter - fluxes.rSoil) * climate->length;
#endif
	...
	envi.coarseRootC += (fluxes.coarseRootCreation-fluxes.coarseRootLoss
			-fluxes.rCoarseRoot) * climate->length;
	envi.fineRootC += (fluxes.fineRootCreation-fluxes.fineRootLoss
			-fluxes.rFineRoot) * climate->length;
...
void calculateFluxes() {
	...
	soilWaterFluxes(&(fluxes.fastFlow), &(fluxes.evaporation),
			...
			fluxes.snowMelt, fluxes.transpiration, litterWater, soilWater);
#else
	...
	if (npp > 0) {
		fluxes.coarseRootCreation=(1-params.leafAllocation
				-params.fineRootAllocation)*npp;
	...
	calcRootResp(&fluxes.rFineRoot, params.fineRootQ10,
			params.baseFineRootResp, envi.fineRootC);
...
void updateTrackers(double oldSoilWater) {
	...
	trackers.evapotranspiration = (fluxes.transpiration + fluxes.immedEvap
			+ fluxes.evaporation + fluxes.sublimation) * climate->length;

	trackers.transpiration = (fluxes.transpiration) * climate->length;

	trackers.soilWetnessFrac = (oldSoilWater + envi.soilWater)/(2.0
			*params.soilWHC);
...
void updateState() {
	...
	envi.plantWoodC += (fluxes.photosynthesis + fluxes.woodCreation
			...
			* climate->length;
	envi.plantLeafC += (fluxes.leafCreation - fluxes.leafLitter)
			* climate->length;
	...
	* climate->length;
	...
	envi.soilWater += (fluxes.rain + fluxes.snowMelt - fluxes.immedEvap
			- fluxes.fastFlow - fluxes.evaporation - fluxes.transpiration
			- fluxes.bottomDrainage) * climate->length;
#endif // LITTER_WATER
	...
	if (err != 0) {
		printf(
				"******* Error type %d while trying to add value to NPP mean tracker in sipnet:updateState() *******\n",
				err);
	...
	err
			= addValueToMeanTracker(meanGPP, fluxes.photosynthesis,
					climate->length); // update running mean of GPP
	if (err != 0) {
		printf(
				"******* Error type %d while trying to add value to GPP mean tracker in sipnet:updateState() *******\n",
				err);
...
#if ROOTS
	envi.plantWoodC = (1-params.coarseRootFrac-params.fineRootFrac)
			*params.plantWoodInit;
#else
...
void runModelOutput(FILE *out, OutputItems *outputItems, int printHeader,
	...
	for (currLoc = firstLoc; currLoc <= lastLoc; currLoc++) {
		...
		while (climate != NULL) {
			updateState();
			if (out != NULL)
				outputState(out, currLoc, climate->year, climate->day,
						climate->time);
			if (outputItems != NULL)
...
void runModelNoOut(double **outArray, int numDataTypes, int dataTypeIndices[],
	...
	while (climate != NULL) {
		...
		for (outputNum = 0; outputNum < numDataTypes; outputNum++)
			outArray[step][outputNum]
					= *(outputPtrs[dataTypeIndices[outputNum]]);
...
char **getDataTypeNames() {
	...
	static char *DATA_TYPES[MAX_DATA_TYPES] = { "EVAPOTRANSPIRATION", "NEE",
			"SOIL_WETNESS", "R_SOIL", "ANNUAL_LITTER", "LAI", "PLANT_WOOD_C" };
#endif
